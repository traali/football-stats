...
function LiveBadge() {
    return (
        ...
        >
            ...
            />
            <span className="text-xs font-bold uppercase tracking-[0.1em] text-semantic-red">KÄYNNISSÄ</span>
        </motion.div>
    )
}
        ...
        >
            <div className="p-6 md:p-8 flex flex-col items-center text-center space-y-6">
                ...
                <div className="flex items-center justify-center space-x-4 md:space-x-8 w-full">
                    <Link to={`/team/${match.team_A_id}`} className="flex-1 flex flex-col items-end space-y-2 hover:opacity-80 transition-opacity focus-visible:outline-none focus-visible:ring-2 ring-accent/50 rounded-xl p-1">
                    ...
                    <Link to={`/team/${match.team_B_id}`} className="flex-1 flex flex-col items-start space-y-2 hover:opacity-80 transition-opacity focus-visible:outline-none focus-visible:ring-2 ring-accent/50 rounded-xl p-1">
                ...
                <div className="flex flex-col items-center gap-1">
                    <span className="text-xs font-bold text-text-muted uppercase tracking-widest">
                        {isLive ? 'Käynnissä' : match.status === MATCH_STATUS.PLAYED ? 'Alkamisaika' : 'Ottelu alkaa'}
                    </span>
                    <div className="flex items-center gap-2 text-text-primary">
                        <Calendar className="w-4 h-4 text-accent" />
                        <span className="text-base font-semibold">{match.date}</span>
                        {match.time && !String(match.time).includes("'") && (
                            <>
